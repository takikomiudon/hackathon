...
func Mycontribution(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		...
		rows, err := db.Query("SELECT name, point, message FROM contribution_list JOIN name_list ON contribution_list.contributorid = name_list.nameid WHERE contributorid=?;", nameid)
		if err != nil {
