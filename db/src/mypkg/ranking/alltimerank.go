...
func Alltimerank(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		rows, err := db.Query("SELECT name, SUM(point) FROM contribution_list JOIN name_list ON contribution_list.contributorid=name_list.nameid WHERE NOT deleted_at GROUP BY contributorid ORDER BY SUM(point) DESC;")
		if err != nil {
